...
class Vector2:
    ...
    def __init__(self, x : float = 0.0, y : float = 0.0):
        if (isinstance(x, float) or isinstance(x, int)) and (isinstance(y, float) or isinstance(y, int)): # normalne konstruowanie
...
class Matrix2x2: 
    ...
    def __init__(self, UpRow : Vector2 = None, DownRow : Vector2 = None, LeftCol : Vector2 = None, RightCol : Vector2 = None):
        self.__data = [[0, 0], [0, 0]]
        if (UpRow != None):
            self.setRow(0, UpRow)
        if (DownRow != None):
    ...
    def __mul__(self, v):
        ...
        if isinstance(v, Matrix2x2):                        # mnożenie macierzy przez macierz
            return Matrix2x2(UpRow = [self.getRow(0) * v.getCol(0), self.getRow(0) * v.getCol(1)], DownRow = [self.getRow(1) * v.getCol(0), self.getRow(1) * v.getCol(1)])  
