...
def createSampleBridge():
    ...
    bridge.connections.append(mechanics.Connection(bridge.points[6], bridge.points[5], defMass, defMaxCompression, defCompressionForceRate, defMaxStrech, defStrechForceRate))
    bridge.connections.append(mechanics.Connection(bridge.points[5], bridge.points[4], defMass, defMaxCompression, defCompressionForceRate, defMaxStrech, defStrechForceRate))
...
def test_RenderBridge():
    ...
    for line in model:
        ...
        assert line[2] < 640
        assert line[3] < 480
        draw.line([(line[0], 480-line[1]), (line[2], 480-line[3])], width=5, fill=(int(255*line[4]) + int(255-255*line[4])*256), joint="curve")
